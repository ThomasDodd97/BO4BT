def GridSampler1D_func(NoSD_sca,dims_li):
    ...
    """
    import numpy as np
    delta_sca = dims_li[0].high - dims_li[0].low
    segments_sca = delta_sca / (NoSD_sca - 1)
    s_li = []
    for i in range(NoSD_sca):
        if i == 0:
            s_li.append(dims_li[0].low)
        elif i == 1:
            s_li.append(dims_li[0].low + segments_sca)
        elif i > 1:
...
def PseudorandomSampler1D_func(NoSD_sca,dims_li):
    ...
    """
    import numpy as np
    s_li = np.random.uniform(dims_li[0].low,dims_li[0].high,NoSD_sca)
    s_arr = np.array(s_li)
    return s_arr
...
def QuasirandomSampler1D_func(NoSD_sca,dims_li):
    ...
    while i < MaxAttempts:
        if i == 0:
            low_sca = dims_li[0].low
            high_sca = dims_li[0].high
        if (i % 250) == 0:
            low_sca = dims_li[0].low
            high_sca = dims_li[0].high
        ...
        NoPI_sca,PointsIn_arr = DetectNoPI1D_func(fixed_arr,dims_li[0].low,dims_li[0].high)
...
def GridSampler3D_func(NoS_desired_sca,dimensions_arr):
    ...
    for i,dim_xx in zip(range(dims_sca),dimensions_arr):
        ...
        value_arr = []
        delta_sca = dim_xx.high - dim_xx.low
        step_sca = delta_sca / (dims_sca - 1)
        start_sca = dim_xx.low
...
def DetectNoPI3DCube_func(fixed_s1_arr,fixed_s2_arr,fixed_x1_arr,dims_li):
    ...
    for s1_sca,s2_sca,x1_sca in zip(fixed_s1_arr,fixed_s2_arr,fixed_x1_arr):
        if s1_sca > dims_li[0].low and s1_sca < dims_li[0].high and s2_sca > dims_li[1].low and s2_sca < dims_li[1].high and x1_sca > dims_li[2].low and x1_sca < dims_li[2].high:
            NoPI_sca += 1
...
def QuasirandomSampler3D_func(NoSD_sca,dims_li):
    ...
    high_s1_sca = dims_li[0].high
    high_s2_sca = dims_li[1].high
    high_x1_sca = dims_li[2].high
    low_s1_sca = dims_li[0].low
    low_s2_sca = dims_li[1].low
    low_x1_sca = dims_li[2].low

    delta_s1_sca = dims_li[0].high - dims_li[0].low
    step_s1_sca = delta_s1_sca / 100
    delta_s2_sca = dims_li[1].high - dims_li[1].low
    step_s2_sca = delta_s2_sca / 100
    delta_x1_sca = dims_li[2].high - dims_li[2].low
...
def QuasirandomSampler5D_func(NoSD_sca,dims_li):
    ...
    while i < MaxAttempts:
        if i == 0 or (i % 100) == 0:
            c_low_s1_sca = dims_li[0].low
            c_high_s1_sca = dims_li[0].high
            c_low_s2_sca = dims_li[1].low
            c_high_s2_sca = dims_li[1].high
            c_low_s3_sca = dims_li[2].low
            c_high_s3_sca = dims_li[2].high
        ...
        if len(s1_low_successful_arr) == 25:
            break
        c_low_s1_sca = c_low_s1_sca - (abs(dims_li[0].high - dims_li[0].low))/100
        c_high_s1_sca = c_high_s1_sca + (abs(dims_li[0].high - dims_li[0].low))/100
        c_low_s2_sca = c_low_s2_sca - (abs(dims_li[1].high - dims_li[1].low))/100
        c_high_s2_sca = c_high_s2_sca + (abs(dims_li[1].high - dims_li[1].low))/100
        c_low_s3_sca = c_low_s3_sca - (abs(dims_li[2].high - dims_li[2].low))/100
        c_high_s3_sca = c_high_s3_sca + (abs(dims_li[2].high - dims_li[2].low))/100
        i += 1
    ...
    while i < MaxAttempts:
        if i == 0 or (i % 100) == 0:
            c_low_x1_sca = dims_li[3].low
            c_high_x1_sca = dims_li[3].high
            c_low_y1_sca = dims_li[4].low
            c_high_y1_sca = dims_li[4].high
        sampler_obj = qmc.Sobol(d=5)
        sample_arr = sampler_obj.random_base2(m=base_sca)
        ...
        if len(x1_low_successful_arr) == 25:
            break
        c_low_x1_sca = c_low_x1_sca - (abs(dims_li[3].high - dims_li[3].low))/100
        c_high_x1_sca = c_high_x1_sca + (abs(dims_li[3].high - dims_li[3].low))/100
        c_low_y1_sca = c_low_y1_sca - (abs(dims_li[4].high - dims_li[4].low))/100
        c_high_y1_sca = c_high_y1_sca + (abs(dims_li[4].high - dims_li[4].low))/100
        i += 1
