def OneDimensionStoichConv_func(chem_lis,bdims_lis):
    ...
    for s1_sca in bdims_lis[0]:
        for chemical_obj,stoich in zip(chem_lis,stoich_lis):
            delta_sca = chemical_obj.upper - chemical_obj.lower
            stoich.append((delta_sca * s1_sca) + chemical_obj.lower)
...
def ThreeDimensionStoichConv_func(chem_lis,bdims_lis):
    ...
    for s1_sca,b1_sca in zip(bdims_lis[0],bdims_lis[2]):
        delta_sca = chem_lis[1].upper - chem_lis[1].lower
        b_stoich_lis.append(((delta_sca * s1_sca) + chem_lis[1].lower) * (1 - b1_sca))
    for s2_sca,b1_sca in zip(bdims_lis[1],bdims_lis[2]):
        delta_sca = chem_lis[2].upper - chem_lis[2].lower
        c_stoich_lis.append(((delta_sca * s2_sca) + chem_lis[2].lower) * (b1_sca))
    a_arr = a_stoich_lis
    b_arr = np.array(b_stoich_lis)
...
# Generating basic stoichiometries from strength parameters
    ...
    for s1_sca in bdims_lis[0]:
        delta_sca = chem_lis[1].upper - chem_lis[1].lower
        b_BasicStoich_lis.append((s1_sca * delta_sca) + chem_lis[1].lower)
    for s2_sca in bdims_lis[1]:
        delta_sca = chem_lis[2].upper - chem_lis[2].lower
        c_BasicStoich_lis.append((s2_sca * delta_sca) + chem_lis[2].lower)
    for s3_sca in bdims_lis[2]:
        delta_sca = chem_lis[3].upper - chem_lis[3].lower
        d_BasicStoich_lis.append((s3_sca * delta_sca) + chem_lis[3].lower)
